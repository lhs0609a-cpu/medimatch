...
export default function JourneySidebar({
  completedCount, totalTasks, progress, budgetTotal, budgetSpent,
  targetDate, onDateChange, getPhaseProgress,
}: JourneySidebarProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-card rounded-2xl border border-border p-5 space-y-2">
        <div className="text-sm font-semibold mb-3">단계별 현황</div>
        {phases.map((phase) => {
          const { percent } = getPhaseProgress(phase.id)
          return (
            <div key={phase.id} className="flex items-center gap-2">
              <span className="text-[10px] text-muted-foreground w-16 truncate">{phase.title}</span>
