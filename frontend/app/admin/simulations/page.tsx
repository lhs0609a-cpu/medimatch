    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <TossIcon icon={BarChart3} color="from-blue-500 to-blue-600" shadow="shadow-blue-500/25" size="md" />
          <div>
      ...
      {stats && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100">
              <div className="flex items-center gap-3 mb-2">
                <TossIcon icon={FileText} color="from-blue-500 to-blue-600" shadow="shadow-blue-500/25" size="sm" />
              ...
              <div className="space-y-2">
                {stats.popular_specialties.slice(0, 5).map((s, i) => (
                  <div key={s.specialty} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-blue-100 text-blue-700' : i === 1 ? 'bg-gray-200 text-gray-600' : 'bg-gray-100 text-gray-500'}`}>{i + 1}</span>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} onClick={() => setDetailModal(item)} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer">
                    ...
                    <td className="px-4 py-3 text-center">
                      {item.report_purchased ? <FileText className="w-4 h-4 text-blue-500 mx-auto" /> : <span className="text-gray-300">-</span>}
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto m-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="text-center p-3 bg-blue-50 rounded-xl">
                  <p className="text-xl font-bold text-blue-700">{detailModal.competitor_count ?? '-'}</p>
                  <p className="text-xs text-blue-600">주변 경쟁</p>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {detailModal.report_purchased && (
                  <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">리포트 구매</span>
                )}
