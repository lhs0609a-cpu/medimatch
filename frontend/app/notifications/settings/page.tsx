...
import {
  ArrowLeft, Loader2, TestTube, Bell, Mail, Smartphone,
  MessageCircle, MapPin, MessagesSquare, CreditCard, Users, Megaphone,
} from 'lucide-react'
...
export default function NotificationSettingsPage() {
  ...
  const loadPreferences = async () => {
    setIsLoading(true)
    try { setPreferences(await notificationService.getPreferences()) }
    catch { setPreferences({ email_enabled: true, push_enabled: true, sms_enabled: false, kakao_enabled: false, prospect_alerts: true, chat_messages: true, payment_updates: true, match_updates: true, marketing: false }) }
    finally { setIsLoading(false) }
  }

  const handleToggle = async (key: keyof NotificationPreference) => {
    if (!preferences) return
    const np = { ...preferences, [key]: !preferences[key] }
    setPreferences(np)
    setIsSaving(true)
    try { await notificationService.updatePreferences({ [key]: np[key] }) }
  ...
  const handleTest = async () => {
    ...
    try { const r = await notificationService.sendTestNotification(); setTestResult(`${r.message} (${r.devices_count}개 디바이스)`) }
    catch { setTestResult('테스트 알림 발송 실패') }
  ...
  const renderRow = (i: typeof items[0]) => (
    <div key={i.k} className="flex items-center justify-between py-2">
      <div className="flex items-center gap-3">
        <TossIcon icon={i.icon} color={i.color} size="sm" shadow={i.shadow} />
        <div><p className="font-medium text-gray-900">{i.t}</p><p className="text-sm text-gray-500">{i.d}</p></div>
      </div>
      <Toggle enabled={preferences?.[i.k] ?? i.v} onChange={() => handleToggle(i.k)} />
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-6 max-w-lg">
        <section className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="space-y-4">{items.filter(i => i.sec === 1).map(renderRow)}</div>
        </section>
        <section className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4">알림 유형</h2>
          <div className="space-y-4">
            {items.filter(i => i.sec === 2).map(renderRow)}
            <div className="flex items-center justify-between py-2 border-t pt-4">
              <div className="flex items-center gap-3">
                <TossIcon icon={Megaphone} color="from-amber-500 to-yellow-500" size="sm" shadow="shadow-amber-500/25" />
                <div><p className="font-medium text-gray-900">마케팅 알림</p><p className="text-sm text-gray-500">이벤트/프로모션 정보</p></div>
              </div>
              <Toggle enabled={preferences?.marketing ?? false} onChange={() => handleToggle('marketing')} />
            </div>
          </div>
        </section>
